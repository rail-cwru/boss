...
class BitFlipMiddleTest(unittest.TestCase):
    def setUp(self):
        json_path = '../sample_configs/test_bitflipmiddle.json'
    ...
    def test_update(self):
        self.controller.env.state_val = [1, 1, 1, 0, 0, 1, 1, 1]
        actions = {}
        actions[0] = [3]
        self.controller.env.update(actions)

        # Test a bitflip
        self.assertListEqual(self.controller.env.state_val, [1, 1, 1, 1, 0, 1, 1, 1])
        ...
        actions[0] = [2]
        self.controller.env.update(actions)
        self.assertListEqual(self.controller.env.state_val, [1, 1, 0, 0, 0, 1, 1, 1])
        ...
        actions[0] = [3]
        self.controller.env.update(actions)
        self.assertListEqual(self.controller.env.state_val, [1, 1, 1, 0, 1, 1, 1, 1])

        self.controller.env.state_val = [1, 0, 0, 0, 0, 0, 1, 1]
        actions[0] = [6]
        self.controller.env.update(actions)
        self.assertListEqual(self.controller.env.state_val, [1, 0, 0, 0, 0, 0, 0, 1])

        actions[0] = [7]
        self.controller.env.update(actions)
        self.assertListEqual(self.controller.env.state_val, [1, 0, 0, 0, 0, 0, 0, 0])
