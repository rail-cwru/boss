...
def _valid_taxiworld_tiles(data):
    ...
    for k, v in data.items():
        if int(k) == 0:
            ...
            assert v == 'L', 'Type 7 must always be a Laser'
        else:
...
class HeistLaser(HierarchicalEnvironment):
    """
    Test environment that is modification of Heist environment
    Not used in paper
    """
    ...
    def get_class_config(cls) -> List['ConfigItemDesc']:
        return [
            ...
            ConfigItemDesc(name='agent_spawn_possibilities',
                           ...
                           optional=True,
                           default=[[2, 4]]),
            ConfigItemDesc(name="clear_carry",
                           check=boolean_check,
    ...
    def __init__(self, config: 'Config'):
        ...
        self.clear_carry_penalty = config.clear_carry
        print("Carry Penalty", self.clear_carry_penalty)
        ...
        self._flat = True
        super(HeistLaser, self).__init__(config)
    ...
    def _set_environment_init_state(self) -> np.ndarray:
        ...
        self.laser_loc = np.argwhere(self.map == 7).tolist()
    ...
    def observe(self, obs_groups=None) -> Dict[Union[int, Tuple[int, ...]], np.ndarray]:
        ...
        crouch = self._observation_domain.index_for_name(name='crouch')
        pre_observation[crouch] = self.crouch
    ...
    def _create_observation_domains(self, config) -> Dict[int, ObservationDomain]:
        ...
        crouch = BinaryFeature(name='crouch')
        items.extend([crouch])
    ...
    def update(self, actions: Dict[int, np.ndarray]) -> Dict[int, int]:
        ...
        for agent in actions:
            ...
            in_lasers = self.in_lasers(agent)
            if action < 4:
                ...
                # if in lasers, the agent can only move if crouching else penalty and empty hands
                elif current_position in self.laser_loc and not self.crouch:
                    # target_position = current_position
                    reward = -15
                    # self.clear_carry()

                elif target_position in self.laser_loc and not self.crouch:
                    reward = -15
                    # self.clear_carry()
            ...
            # CROUCH
                ...
                # print('Crouch')
                # Agent just stood up and set off lasers
                if in_lasers and self.crouch == 1:
                    reward = -15
                    if self.clear_carry_penalty:
                        self.clear_carry()
                    # print('CC')

                # flip crouch
                self.crouch = (self.crouch+1)%2
    ...
    def is_carry(self):
        return ((self.bk == len(self.bk_possible_loc) and self.bl < len(self.bl_possible_loc)) or
                (self.gk == len(self.gk_possible_loc) and self.gl < len(self.gl_possible_loc)) or
                (self.ok == len(self.ok_possible_loc) and self.ol < len(self.ol_possible_loc)))

    def clear_carry(self):
        '''
        Returns anything agent is holding
        :return:
        '''
        if self.bk == len(self.bk_possible_loc) and self.bl < len(self.bl_possible_loc):
            self.bk = 0
        elif self.gk == len(self.gk_possible_loc) and self.gl < len(self.gl_possible_loc):
            self.gk = 0
        elif self.ok == len(self.ok_possible_loc) and self.ol < len(self.ol_possible_loc):
            self.ok = 0

    def in_lasers(self, agent) -> bool:
        return self._agent_positions[agent] in self.laser_loc

    def successfully_pickup_blue_key(self, agent) -> bool:
        return (not self.is_carry()) and self.bk_loc == self._agent_positions[agent] and self.bk < len(self.bk_possible_loc)

    def successfully_pickup_green_key(self, agent) -> bool:
        return (not self.is_carry()) and self.gk_loc == self._agent_positions[agent] and self.gk < len(self.gk_possible_loc)

    def successfully_pickup_orange_key(self, agent) -> bool:
        return (not self.is_carry()) and self.ok_loc == self._agent_positions[agent] and self.ok < len(self.ok_possible_loc)
    ...
    def successfully_pickup_gem(self, agent) -> bool:
        ...
        return self.gem_loc == self._agent_positions[agent] and self.ol == len(self.ol_possible_loc)

    def successfully_unlock_blue(self, agent) -> bool:
        location = self.bl_loc
        return (self.bk == len(self.bk_possible_loc) and
                self.bl < len(self.bl_possible_loc) and
                location == self._agent_positions[agent])
