...
class BitFlipPyramidSixteen(BitFlip):

    def __init__(self, config: 'Config'):
        super(BitFlipPyramidSixteen, self).__init__(config)
    ...
    def update(self, actions: Dict[int, np.ndarray]) -> Dict[int, int]:
        ...
        for agent in actions:
            ...
            eights = np.zeros(int(self.num_bits/8))

            # Check twos
            for i in range(int(self.num_bits/2)):
            ...
            eights[0] = sum(fours[0:2])
            eights[1] = sum(fours[2:4])

            action_twos = int(action/2)
            action_fours = int(action/4)
            if action_fours == 0 or action_fours == 1:
                other_action_fours = (action_fours + 1) % 2
            else:
                other_action_fours = 2 + (action_fours + 1) % 2

            action_eights = int(action/8)
            other_action_eights = (action_eights + 1) % 2
            ...
            if eights[other_action_eights] != 0 and eights[action_eights] > eights[other_action_eights]:
                bad_flip_8 = True

            if bad_flip_8:
                start = int(other_action_eights*8)
                end = start + 8
            ...
            if not (bad_flip_4 or bad_flip_2 or bad_flip_8):
