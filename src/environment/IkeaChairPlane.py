...
class IkeaChairPlane(HierarchicalEnvironment):
    ...
    def __init__(self, config: 'Config'):
        ...
        self.stabilizer_x = 0
        self.claw_x = 0
        self.claw_holding = 4 # Empty handed
        ...
        config.seed = 2
        np.random.seed()
        super(IkeaChairPlane, self).__init__(config)
    ...
    def _set_environment_init_state(self) -> np.ndarray:
        ...
        self.state[4] = self.claw_x

        self.claw_holding = 4
        self.state[5] = self.claw_holding
    ...
    def update(self, actions: Dict[int, np.ndarray]) -> Dict[int, int]:
        ...
        for agent in actions:
            ...
            if action < 4:
                ...
                previous_connected = self.parts_x_values[0] == self.leg_x_vals - 1 and all([ i ==  IkeaChairPlane.Location.CONNECTED.value for i in self.parts_x_values[1:action]])
                ...
                if action == 0:
                    already_connected = self.parts_x_values[action] == self.leg_x_vals - 1
                else:
                    already_connected = self.parts_x_values[action] == IkeaChairPlane.Location.CONNECTED.value
                ...
                if claw_holding and self.claw_x == 0 and not already_connected and (action == 0 or previous_connected):
                    # self.parts_x_values[action] = -1
                    if action == 0:
                        self.parts_x_values[action] = self.leg_x_vals - 1
                    else:
                        self.parts_x_values[action] = IkeaChairPlane.Location.CONNECTED.value

                    # print("Connected", action)

                    reward = 1
                    self.claw_holding = 4
            elif self.single_pickup and action == 4:
            ...
            elif not self.single_pickup and 3<action<8:
                # Pickup Legs
                if action == 4:
                    # Claw at legs
                    claw_at_legs = self.claw_x == self.parts_x_values[0] and self.parts_x_values[0] < self.leg_x_vals - 1
                    ...
                    if self.claw_holding == 4 and claw_at_legs:
                ...
                # Pickup cushion
                    ...
                    # does not update location, instead will keep "old" position to return it
                    if self.claw_holding == 4 and self.claw_x == self.parts_x_values[2]:
                        self.claw_holding = 2
                        # print('Pickup', self.claw_holding)
                    else:
                        reward = -5
                ...
                # Pickup back
                    ...
                    # does not update location, instead will keep "old" position to return it
                    if self.claw_holding == 4 and self.claw_x == self.parts_x_values[3]:
                        self.claw_holding = 3
                        # print('Pickup', self.claw_holding)
                    else:
                        reward = -5
            ...
            elif not self.single_putdown and ((not self.single_pickup and 7<action<12) or (self.single_pickup and 4<action<9)):

                action_sub = 3 if self.single_pickup else 0

                # Putdown legs
                # (will only be allowed to place back at the objects original location!)
                if action == (8 - action_sub) :
                    if self.claw_holding == 0 and self.claw_x == self.parts_x_values[self.claw_holding]:
                        # print('Put down', self.claw_holding)
                        self.claw_holding = 4
                    else:
                        reward = -5
                ...
                # Putdown cush
                # (will only be allowed to place back at the objects original location!)
                elif action == (10 - action_sub):
                    if self.claw_holding == 2 and self.claw_x == self.parts_x_values[self.claw_holding]:
                        # print('Put down', self.claw_holding)
                        self.claw_holding = 4
                    else:
                        reward = -5

                elif action == (11 - action_sub):
                    if self.claw_holding == 3 and self.claw_x == self.parts_x_values[self.claw_holding]:
                        # print('Put down', self.claw_holding)
                        self.claw_holding = 4
                    else:
                        reward = -5
        ...
        #     self.soft_reset()
            ...
            d = True
            for i in self.parts_x_values[1:]:
                if i != IkeaChairPlane.Location.CONNECTED.value or self.parts_x_values[0] != self.leg_x_vals - 1:
                    d = False
            self.done = d
            if d:
    ...
    def make_state(self):
        ...
        # state[3] = self.leg_x
        state[0:4] = self.parts_x_values
        state[4] = self.claw_x
        state[5] = self.claw_holding
        return state
    ...
    def observe(self, obs_groups=None) -> Dict[Union[int, Tuple[int, ...]], np.ndarray]:
        ...
        pre_observation[a_cushion_slice] = self.parts_x_values[2]


        a_back_slice = self._observation_domain.index_for_name(name='back_loc')
        pre_observation[a_back_slice] = self.parts_x_values[3]
